...
def video_test(input_video_path=None, output_video_path=None):
    ...
    while(ret):
        ...
        cv2.imshow(
            'img',
            np.concatenate((img, angle_map, dist_map, result_img), axis=1))

        dst = regress_perspecive(result_img, pts1, (height, width))
        dst = cv2.addWeighted(frame, 0.3, dst, 0.7, 0)
        cv2.imshow(
            'inv', dst)

        out.write(np.concatenate((img, angle_map, dist_map, result_img), axis=1))
